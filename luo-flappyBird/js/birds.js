...
Bird.prototype.draw = function (){
    ...
    if(this.speed===0){
        ...
        this.ctx.rotate((Math.PI /2) * this.speed*0.7 / (0.5*this.proportion)+Math.PI /2);
    }else {
        this.ctx.rotate((Math.PI /2)+Math.PI /2);
    }
/*    this.ctx.drawImage(
    ...
    this.ctx.drawImage(
        this.img,this.img.width*1/6,this.img.width*1/8,this.img.width*4/6,this.img.width,
        -this.img.width/25*this.proportion,-this.img.width/25*this.proportion,this.img.width/25*2*this.proportion,this.img.width/25*2*6/4*this.proportion  //这里很重要的一点是，整个小鸟坐标系开始移动
    );
    this.ctx.restore();
};
